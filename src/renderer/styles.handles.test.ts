 ...
 * therefore REPLACES that translate — which is how the bridge/pipe dots' hover "scale" shifted
 * each dot half its size away from the cursor (and could push it out from under the pointer,
  ...
  it('finds no transform inside a handle-targeting rule block', () => {
    ...
    lines.forEach((line, i) => {
      if (line.includes('{') && !line.trim().startsWith('@')) {
        ...
        inHandleBlock = /(^|[.\s,])(react-flow__handle|bridge-handle|pipe-handle)/.test(selector)
      }
